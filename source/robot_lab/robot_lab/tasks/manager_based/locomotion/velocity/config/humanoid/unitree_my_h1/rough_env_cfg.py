...
# Pre-defined configs
##
from robot_lab.assets.my_h1 import UNITREE_My_H1_ACTION_SCALE, UNITREE_My_H1_CFG  # isort: skip


@configclass
class UnitreeMyH1RoughEnvCfg(LocomotionVelocityRoughEnvCfg):
    base_link_name = "torso_link"
    foot_link_name = ".*_ankle_link"
    # fmt: off
    # joint_names = [
    ...
    def __post_init__(self):
        ...
        # ------------------------------Sence------------------------------
        self.scene.robot = UNITREE_My_H1_CFG.replace(prim_path="{ENV_REGEX_NS}/Robot")
        ...
        # Run 11：关 base_lin_vel obs（policy 66 维，对齐官方）；play 须用本 run 的 ckpt，勿 load Run10（69 维）
        ...
        # ------------------------------Actions------------------------------
        # Run 12：统一 action scale 0.25（一次只改此项）；play 须用本 run ckpt（66 维 obs 同 Run11）
        self.actions.joint_pos.scale = 0.25
        # self.actions.joint_pos.scale = UNITREE_My_H1_ACTION_SCALE
        ...
        self.rewards.create_joint_deviation_l1_rewterm("joint_deviation_hip_l1", -0.2, [".*hip_yaw.*", ".*hip_roll.*"])
        self.rewards.create_joint_deviation_l1_rewterm("joint_deviation_arms_l1", -0.2, [".*shoulder.*", ".*elbow.*"])
        self.rewards.create_joint_deviation_l1_rewterm("joint_deviation_torso_l1", -0.2, ["torso_joint"])
        ...
        # ------------------------------Commands------------------------------
        self.commands.base_velocity.ranges.lin_vel_x = (0, 0.3)
